...
use crate::models::artist::{ActiveModel, Entity, Model, ModelEx, };
...
impl From<ArtistCreate> for ActiveModel {
    fn from(source: ArtistCreate) -> Self {
        ActiveModel {
...
pub async fn list(state: State<AppState>) -> Result<impl IntoResponse, (StatusCode, Json<serde_json::Value>)> {
    let models = Entity::load()
        ...
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": e.to_string()}))))?;
    let responses: Vec<Artist> = models.into_iter().map(Into::into).collect();

    Ok(Json(responses))
}

pub async fn create(state: State<AppState>, Json(create): Json<ArtistCreate>) -> Result<impl IntoResponse, (StatusCode, Json<serde_json::Value>)> {
    ...
    let model = active_model.insert(&state.db)
        ...
        let resp: Artist = model.into();
        Ok(Json(resp))

}

pub async fn patch_one(state: State<AppState>, Path(id): Path<i32>, Json(patch): Json<ArtistPatch> ) -> Result<impl IntoResponse, (StatusCode, Json<serde_json::Value>)> {
...
pub async fn put_one(state: State<AppState>, Path(id): Path<i32>, Json(update): Json<ArtistUpdate>) -> Result<impl IntoResponse, (StatusCode, Json<serde_json::Value>)> {
...
pub fn routes() -> Router<AppState> {
    Router::new()
        .route("/artists", get(list))
        .route("/artists", post(create))
        .route("/artists/{id}", get(read_one))
        .route("/artists/{id}", delete(remove))
        .route("/artists/{id}", patch(patch_one))
        .route("/artists/{id}", put(put_one))
}